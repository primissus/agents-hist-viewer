...
10. **MCP tool call** (`internal/adapter/mcp`, `chv mcp`): `cmdMCP` wires `Deps{Search, Semantic, Summarize, Patterns, Ask}` from the same services above and calls `mcpadapter.Run`, which starts an SDK stdio server; each of the 7 registered tools (`search_history`, `semantic_search`, `list_sessions`, `get_session`, `summarize_session`, `mine_patterns`, `ask_history`) is a thin handler translating tool-call input into the corresponding `app` service call and back into a JSON DTO — no logic lives in the adapter beyond that translation and clamping (limits, kinds) already tolerant of missing deps (e.g. Ollama down → `semantic_search`/`summarize_session`/`ask_history` return a tool error, not a crash). `mine_patterns`/`ask_history` default to `label_with_llm`/`answer` false (retrieval only, no chat model call) — the calling MCP client is the reasoner.
